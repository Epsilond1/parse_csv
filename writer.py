...
class Writer(object):
    ...
    def record(self):
        ...
        sheet.write(0, 1, 'Описание', style0)
        sheet.write(0, 2, 'Хар-ка элементов', style0)
        sheet.write(0, 3, 'Количество элементов', style0)

        # Заполнение документа
        row_num = 1
        for element in self.info_element:
            sheet.write(row_num, 0, '{0} - {1}'.format(element[1], element[2]), style0)
            sheet.write(row_num, 2, '{0}'.format(element[0].character), style0)
            sheet.write(row_num, 3, '{0}'.format(element[0].count), style0)
            row_num += 1
